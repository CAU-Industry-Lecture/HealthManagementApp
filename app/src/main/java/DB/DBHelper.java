...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void addExerciseData() {
        ...
        for(int i = 0 ; i < 2 ; i++){
            ...
            sb.append(" exe_name, exe_cate, exe_cal, count_all, count_now, interest) ");
            sb.append(" VALUES ( ?, ?, ?, ?, ?, ?) ");
            try{
                instance.execSQL(sb.toString(), new Object[]{exename[i], execate[i], execal[i], 0, 0, "NO"});
                Log.d("t","생성완료3");
            } catch (Exception e){
                Log.d("t","오류3");
            }
    ...
    public List getAllExerciseData() {
        StringBuffer sb = new StringBuffer();
        sb.append(" SELECT exe_name, exe_cate, exe_cal, count_all, count_now, interest FROM EXERCISE ");
        ...
        while (cursor.moveToNext()) {
            ...
            exercise.setCount_all(cursor.getInt(3));
            exercise.setCount_now(cursor.getInt(4));
            exercise.setInterest(cursor.getString(5));
    ...
    public void updateInterest(Map<String, Integer> exerciseType){
        // 1이면 YES로 업데이트
